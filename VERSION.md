...
## dev
- STRM 生成
  - **云盘类存储生成支持 `文件编号模式`**：以文件编号替代原路径方式写入 STRM 内容，减少API请求，可稍微提高起播速度（推荐）；改模式后重新生成建议使用以下 `一键覆写` 功能
- 驱动
...
- 任务工具
  - 增加 `一键覆写` 功能：免删除强制覆盖生成，避免媒体库中媒体被自动移除
...
> 因为这次改动较多，对所有驱动都做了重构，所以暂且发布一个 dev 版本，算是测试版，有兴趣的老哥可以拉 :dev 的 tag 体验。有问题欢迎反馈，最好是一次性组织好语言，而非三言两语；没问题也欢迎反馈让我知道，谢谢大家！
